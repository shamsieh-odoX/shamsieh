...
Broken compute had newlines collapsed, e.g.:
  record['x_ssc_employer_share_report'] = amountfor record in self:
"""
...
def migrate(cr, version):
    ...
    rows = cr.fetchall()
    for field_id, compute in rows:
        text = compute or ''
        if 'amountfor' in text.replace(' ', '') or 'amountfor' in text:
            cr.execute(
                """
